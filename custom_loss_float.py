...
def compute_log_likelihood_using_conditional_density_no_causality(
    target,
    A_coeffs_from_lstm,
...
var_cov_params_for_initial_obs
    ...
    for t in range(order,len_of_seq):
        ...
        A_coeffs=A_coeffs_from_lstm[t,0,:].reshape(m, m, order)
        # print('A_coeffs')
        # print(A_coeffs)
...
def compute_log_likelihood_using_conditional_density_no_causality_gls(
    target,
    A_coeffs_from_lstm,
    #lower_triang_params_form_lstm shape:(seq,batch_size=1,m*(m+1)/2)
    lower_triang_params_form_lstm,
...
var_cov_params_for_initial_obs
    ...
    for t in range(order,len_of_seq):
        ...
        lagged_obs=get_lagged_observations(target, t, order, m)
        error=target[t,:].reshape(m, 1)-torch.mm(A,lagged_obs)
        log_likelihood_temp=log_likelihood_temp+torch.mm(
            torch.mm(error.t(), torch.inverse(var_cov_innovations_varp)), error)
...
def compute_log_likelihood_using_conditional_density(
    target,
    A_coeffs_from_lstm,
    #lower_triang_params_form_lstm shape:(seq,batch_size=1,m*(m+1)/2)
    lower_triang_params_form_lstm,
...
def compute_log_likelihood_using_diff_decop_for_covariance(
    target,
    A_coeffs_from_lstm,
    #lower_triang_params_form_lstm shape:(seq,batch_size=1,m*(m+1)/2)
    lower_triang_params_form_lstm,
...
var_cov_params_for_initial_obs
    ...
    for t in range(order,len_of_seq):
        #calculate var-cov of innovations of var(p)
        var_cov_innovations_varp=make_var_cov_matrix_for_innovation_of_varp_three_decomp(lower_triang_params_form_lstm[t,0, :], m, order)
...
def compute_log_likelihood_using_conditional_density_with_kalman(
    target,
    A_coeffs_from_lstm,
    #lower_triang_params_form_lstm shape:(seq,batch_size=1,m*(m+1)/2)
    lower_triang_params_form_lstm,
...
var_cov_params_for_initial_obs
    ...
    # coeff_parameters=[]
    # var_cov_parameters=[]
    X0 = torch.zeros(order*m, 1)
    v_covariance_coeffs=var_cov_params_for_initial_obs
    #emssion matrix
    G=make_G_m(m,order)
    #P0 = make_covariance_matrix_p(v_covariance_coeffs[0,:],m,lag_order)
    P0 = make_covariance_matrix_p(v_covariance_coeffs,m,order)
    for t in range(order):
        # print('t')
        # print(t+1)
        
        #F_temp: shape()
        # A_coeffs = Variable(A_coeffs_for_causal_VAR(F_out_network=F_t[t,0,:], p=lag_order, d=m),
        #                             requires_grad=True)
        # here Q variance matrix is time variant
        var_cov_innovations_var1 = make_var_cov_of_innovations_var1(lower_triang_params_form_lstm[t,0, :], m, order)
        # print('Q_covariance')
        # print(Q_covariance)
        var_cov_innovations_varp=make_var_cov_matrix_for_innovation_of_varp(lower_triang_params_form_lstm[t,0, :], m, order)
        lower_t_for_innovations_varp=get_lower_trang_m(lower_triang_params_form_lstm[t,0, :], m, order)
        ...
        X1 = torch.mm(A_coeffs_var1, X0)
        #P1 = (torch.mm(torch.mm(A_coeffs_var1, P0), A_coeffs_var1.t()) + var_cov_innovations_var1).float()
        P1 = (torch.mm(torch.mm(A_coeffs_var1, P0), A_coeffs_var1.t()) + var_cov_innovations_var1)
        # print('P1_e')
        # print(P1)
        # print(torch.eig(P1, eigenvectors=False, out=None))
        # P1=mx.nd.dot(mx.nd.dot(FF,P0),FF.transpose())+Q
        # Y_t - E[Y_t|Y_ 1,...,Y_t-1]
        e = torch.mm(G, X1)
        # print('e_value')
        # print(e)
        # e=mx.nd.dot(G,X1.astype('float64'))
        ##targets: shape(k*T)
        # print('target')
        # print(target[:,t].shape)
        E1 = target[t,:].reshape(m, 1) - e
        # print('target')
        # print(target[ts,t,:])
        # print('target_shape')
        # print(target[ts,t,:].shape)
        # E1=nd.array(target[ts,t,:].asnumpy().reshape(m,1)).astype('float64')-e
        # Var{Y_t - E[Y_t|Y_ 1,...,Y_t-1]}
        # SE=torch.mm(torch.mm(G,P1),G.t())+R
        SE = torch.mm(torch.mm(G, P1), G.t())
        # update 20200416
        K = torch.mm(torch.mm(P1, G.t()), torch.inverse(SE))
        X0 = X1 + torch.mm(K, E1)
        P0 = P1 - torch.mm(torch.mm(K, G), P1)
        SE_det = torch.det(SE)
        # print('SE_det')
        # print(SE_det)
        # update 20200416
        # add penalty
        # y_t_plus_trend_t=target[ts,t,:].reshape(m,1)-trend.reshape(m,1)
        # print('y_t_plus_trend_t')
        # print(y_t_plus_trend_t.shape)
        # log_p_for_one_ts_temp=log_p_for_one_ts_temp+(0.5)*torch.mm(torch.transpose(E1,0,1),Z)+get_diag_elements(SE_L)
        # log_p_for_one_ts_temp = log_p_for_one_ts_temp + (0.5) * torch.mm(torch.transpose(E1, 0, 1),Z) + torch.sum(a)
        log_likelihood_temp = log_likelihood_temp + torch.log(SE_det) + torch.mm(
            torch.mm(E1.t(), torch.inverse(SE)), E1)
        #calculate
...
def compute_log_likelihood_check_causality(
    target,
    A_coeffs_from_lstm,
    #lower_triang_params_form_lstm shape:(seq,batch_size=1,m*(m+1)/2)
    lower_triang_params_form_lstm,
...
var_cov_params_for_initial_obs
    ...
    for t in range(order,len_of_seq):
        ...
        # print(A_coeffs)
        #check casuality
        res=check_causality(A_coeffs, m, order)
        if res:
            A_coeffs = A_coeffs_for_causal_VAR(A_coeffs_from_lstm[t,0,:], order, m,lower_t_for_innovations_varp)
